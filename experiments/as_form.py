...
from fastapi import Form, Body, Query, FastAPI, Depends
...
class CommonQueryParams(BaseModel):
    ...
    redirect_uri: Optional[str] = Field(None, description="Deprecated: URL to redirect to (use redirect_url)")
    ...
    def as_form(
        ...
        return cls(client_id=client_id, state=state, scope=scope, redirect_url=redirect_url,
...
@app.post("/query-form")
def post(form_data: CommonQueryParams = Depends(CommonQueryParams.as_form),
...
@app.post("/query-json")
def post2(json_data: CommonQueryParams = Depends(CommonQueryParams.as_json),
          query_data: CommonQueryParams = Depends(CommonQueryParams.as_query)):
